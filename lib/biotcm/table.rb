...
class BioTCM::Table
  ...
  # @private
  def self._build(primary_key, row_keys, col_keys, content)
    tab = self.new
  ...
  def initialize(arg=nil)
    ...
    when nil # Empty table
      ...
      @content = []
    when String
      initialize_by_string(File.open(arg).read)
    else
  ...
  def initialize_by_string(str)
    ...
    @row_keys = []
    @content = []
    content.each_with_index do |line, line_index|
...
class String
  ...
  def to_table
    BioTCM::Table.new.send(:initialize_by_string, self)
  end
end
