...
class BioTCM::Databases::KEGG
  ...
  def self.validate_pathway_id(pathway_id, organism = DEFAULT_ORGANISM)
    fail ArgumentError, 'Invalid organism' unless organism =~ PATTERNS[:organism]
    case pathway_id
  ...
  def self.get_pathway_list(organism = DEFAULT_ORGANISM)
    fail ArgumentError, 'Invalid organism' unless organism =~ PATTERNS[:organism]
    file_path = path_to("list_#{organism}.txt")
    unless File.exist?(file_path)
      fout = File.open(file_path, 'w')
      fout.puts BioTCM.get(URLS[:pathway_list].call(organism))
      fout.close
    end
    list = []
  ...
  def self.get_pathway(pathway_id)
    fail ArgumentError, 'Invalid pathway_id' unless (pathway_id = validate_pathway_id(pathway_id))
...
BioTCM::Databases::KEGG.wd = BioTCM.path_to('data/kegg')
