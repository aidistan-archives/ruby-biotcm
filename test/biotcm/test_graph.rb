...
describe Graph do
  ...
  describe 'when initialized with invalid edges' do
    it 'must raise ArgumentError' do
      ...
      file.flush
      assert_raises(ArgumentError) { Graph.new(file.path) }
      file.close!
    end
  end
  ...
  describe 'when method called' do
    ...
    it 'must return sub-graph given selected nodes' do
      assert_equal([], @net.select(%w(1 3 8)).edge)
      assert_equal(['1--2', '2--3', '3--4', '4--1'], @net.select(%w(1 2 3 4)).edge)
    end

    it 'must expand the graph given selected nodes' do
      assert_equal(@net.edge, @net.select(%w(1 3 8)).expand.edge)
    end

    it 'must knock down edges connected to selected nodes' do
      assert_equal(['1--2', '4--1', '1--5'], @net.knock_down(%w(3 8)).edge)
    end
  end
end
